...
namespace phd::text { inline namespace PHD_TEXT_ABI_NAMESPACE {
	...
	struct wide_execution {
		...
		static constexpr auto encode(
			...
			auto __result = __u16enc.encode(std::forward<_InputRange>(__input), std::forward<_OutputRange>(__output),
			...
			if (__result.error_code != encoding_errc::ok) {
				return __error_handler(wide_execution{},
					__result_t(std::move(__result.input), std::move(__result.output), __s, __result.error_code));
			}
			...
			execution __exec{};
			auto __result = __exec.encode(std::forward<_InputRange>(__input), std::span(__pray_start, __state_max),
				__s.narrow_state, __detail::__pass_through_text_error_handler_with<!__call_error_handler>{});
			if constexpr (__call_error_handler) {
				if (__result.error_code != encoding_errc::ok) {
					return __error_handler(wide_execution{},
						__result_t(std::move(__result.input), _UOutputRange(__outit, __outlast), __s,
						     __result.error_code));
				}
			}
			...
			return __result_t(
				std::move(__result.input), _UOutputRange(__outit, __outlast), __s, __result.error_code);
#endif // Windows shit
		}
		...
		static constexpr auto decode(
			...
			__u16e __u16enc{};
			__intermediate_state __intermediate_s{};
			auto __result = __u16enc.decode(std::forward<_InputRange>(__input), std::forward<_OutputRange>(__output),
				__intermediate_s, __detail::__pass_through_text_error_handler_with<!__call_error_handler>{});
			if constexpr (__call_error_handler) {
				if (__result.error_code != encoding_errc::ok) {
					return __error_handler(wide_execution{},
...
#ifdef _MSC_VER
				std::size_t __res;
				errno_t __err = wcrtomb_s(
					std::addressof(__res), __pray_for_state, __state_max, __unit, std::addressof(__s.wide_state));
				if constexpr (__call_error_handler) {
					if (__err != 0) {
						// error: cry about it
						return __error_handler(wide_execution{},
			...
			if constexpr (__call_error_handler) {
				if (__result.error_code != encoding_errc::ok) {
					return __error_handler(wide_execution{},
						__result_t(_UInputRange(__init, __inlast), std::move(__result.output), __s,
						     __result.error_code));
				}
			}
			return __result_t(_UInputRange(__init, __inlast), std::move(__result.output), __s, __result.error_code);
#endif
		}
	};
