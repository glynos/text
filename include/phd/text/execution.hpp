	...
	struct execution {
		...
		static constexpr auto encode(
			...
			if (__detail::windows::__determine_active_code_page() == CP_UTF8) {
				...
				__detail::windows::BOOL __used_default_char = false;
				...
				return __result_t(std::move(__intermediate_result.input), _UOutputRange(__outit, __outlast), __s,
					__intermediate_result.error_code);
			}
#else
		...
		static constexpr auto decode(
			...
			for (; __state_count < __state_count_max; ++__state_count) {
				...
				return __result_t(_UInputRange(__init, __inlast), std::move(__intermediate_result.output), __s,
					__intermediate_result.error_code);
#else
